...
def config_search(z, yf, z_all, y_all, Q_shared, e_prev, n_k, config_steps, config_margin,
    ...
    for inter, kp in cands:
        ...
        results = train_batch([(z_all, y_all)], inter, config_steps, kp=kp,
                              Q_shared=Q_shared,
...
def train_batch(pairs, inter, steps, kp=KP, Q_shared=None, check_every=25, patience=4000, stop_threshold=None, n_real=0, use_compile=False,
    ...
    for st in range(steps):
        ...
        if use_compile and ((st + 1) % check_every == 0 or st == 0):
            ...
            if not (torch.isfinite(W1.grad).all() and torch.isfinite(W3.grad).all()
                    and torch.isfinite(W2.grad).all()):
                raise RuntimeError(
    ...
    out = []
    for i in range(G):
        out.append((w1q[i], w1s[i], w1q2[i], w1s2[i],
                    w3q[i], w3s[i], w3q2[i], w3s2[i],
                    w2q[i], w2s[i], w2q2[i], w2s2[i]))
    return out
...
def warm_init_from(e, inter, kp):
    ...
    try:
        if 'w1' not in e or 'w3' not in e or 'w2' not in e or e.get('inter') != inter:
            return None
        w1 = (unpack_ternary(e['w1']).float() * e['w1_scale'].float()[:, None] +
              unpack_ternary(e['w1_q2']).float() * e['w1_scale2'].float()[:, None])
        w3 = (unpack_ternary(e['w3']).float() * e['w3_scale'].float()[:, None] +
              unpack_ternary(e['w3_q2']).float() * e['w3_scale2'].float()[:, None])
        w2 = (unpack_ternary(e['w2']).float() * e['w2_scale'].float()[:, None] +
        ...
        for (k, z, yf, e_prev) in covered_todo:
            ...
            if mode == 'config':
                ...
                inter, kp, res, resid = config_search(
                    z, yf, z_all, y_all, Q_shared, e_prev, n_k, config_steps, config_margin,
                    tier_thr, use_compile, warm, rw_eff)
                ...
                init = warm_init_from(e_prev, inter, kp) if (warm and e_prev is not None) else fp4_init
                results = train_batch([(z_all, y_all)], inter, tier_steps, kp=kp,
                                      Q_shared=None,
                                      stop_threshold=tier_thr, n_real=z.shape[0], use_compile=use_compile,
                                      init=init, real_weight=rw_eff)
                w1q, w1s, w1q2, w1s2, w3q, w3s, w3q2, w3s2, w2q, w2s, w2q2, w2s2 = results[0]
                resid = resid_weights_full(z, yf, None, w1q, w1s, w1q2, w1s2, w3q, w3s, w3q2, w3s2, w2q, w2s, w2q2, w2s2)
            save_expert(k, resid, w1q, w1s, w1q2, w1s2, w3q, w3s, w3q2, w3s2, w2q, w2s, w2q2, w2s2, inter, n_real=n_k)
            n_fixed += 1
