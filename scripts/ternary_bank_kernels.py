...
def k_h13t(
    ...
    for g in range(0, NG):
        ...
        sg1 = tl.load(s13_ptr + e * (2 * I * NG) + offs_i * NG + g)
        sg3 = tl.load(s13_ptr + e * (2 * I * NG) + (I + offs_i) * NG + g)
        acc_g += tl.sum(t1 * zf[None, :], axis=1) * sg1
        acc_u += tl.sum(t3 * zf[None, :], axis=1) * sg3
    bv1 = tl.load(b13_ptr + e * (2 * I) + offs_i)
    bv3 = tl.load(b13_ptr + e * (2 * I) + I + offs_i)
    gv = _soft_lim(acc_g + bv1, 10.0, 0.75)
    uv = _soft_lim(acc_u + bv3, 10.0, 0.75)
    h = gv * tl.sigmoid(gv) * uv
