...
class DistillationTeacher:
    ...
    def distillation_loss_chunked(
        ...
        when ``alpha >= 1.0`` the CE signal dominates entirely and the
        MSE term contributes nothing; when ``alpha <= 0.0`` the function
        returns ``reconstruction_loss`` unchanged (CE-only). This is the
        theoretically-correct behavior — the naive ``alpha * CE + (1-alpha) *
        MSE`` would, at ``alpha = 0``, invert to ``1 * MSE`` and silently
        replace CE with hidden alignment. Setting ``alpha = 0`` disables
        hidden alignment entirely (it was found to compete with the CE signal
        via distribution mismatch); this early-return makes that intent
        explicit and avoids the inversion.
        """
        if teacher_hidden is None:
            return reconstruction_loss

        # alpha <= 0 means "CE only, no alignment". Returning early avoids the
        # ``0 * CE + 1 * MSE`` inversion that silently replaced the primary
        # objective with hidden-state matching.
        if alpha <= 0.0:
            return reconstruction_loss
