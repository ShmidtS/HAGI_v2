# V21 ARCHIVE: V6-era module, not wired in V21 forward path.
# Kept for design reference ([Chesterton's Fence]). See docs/ARCHITECTURE.md.
# DO NOT import from codec/ package — these are legacy.

"""HRM тАФ Spatial Refinement Controller for V4 plane prediction.

Key differences from V1/V3:
- z_H: [B, T//stride, h_dim] тАФ spatial plane (not single vector)
- z_L: [B, T, l_dim] тАФ per-token refinement state
- Iterations replace l_cycles
- NO h.detach() тАФ gradient checkpointing per iteration instead
- Deep supervision at each iteration (NOT detached тАФ provides gradient)
- Adaptive halting: scalar grade confidence тЖТ halt decision
"""
...
class RefinementCore(nn.Module):
    ...
    def forward(
        ...
        ext_norm_acc = h.new_zeros(())  # GPU accumulator тАФ 1 sync at end
