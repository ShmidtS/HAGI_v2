...
class RefinementCore(nn.Module):
    def __init__(
        ...
        self.use_adaptive_ds_weight = refinement_cfg.use_adaptive_ds_weight
        self.ds_ema_decay = refinement_cfg.ds_ema_decay
        self.use_entropy_adaptive_refinement = refinement_cfg.use_entropy_adaptive_refinement
        self.entropy_low_threshold = refinement_cfg.entropy_low_threshold
        self.entropy_high_threshold = refinement_cfg.entropy_high_threshold
        self.entropy_low_iterations = refinement_cfg.entropy_low_iterations
        self.entropy_high_iterations = refinement_cfg.entropy_high_iterations
    ...
    def _compute_n_iterations(self, h: torch.Tensor) -> int:
        if not self.use_entropy_adaptive_refinement:
    ...
    def forward(
        ...
        n_iters = self._compute_n_iterations(h)
        ...
        for iteration in range(n_iters):
            ...
            if training and self.use_deep_supervision and targets is not None and mask is not None and mask.any():
                ...
                if self.use_adaptive_ds_weight:
                    ...
                    self._ds_ema[iteration] = (
                        self.ds_ema_decay * self._ds_ema[iteration] + (1 - self.ds_ema_decay) * delta_norm
                    )
