...
class TestLossAggregator:
    ...
    def test_adds_rate(self, agg):
        loss = agg(_out(2.0, rate=torch.tensor(1.0)), step=0)
        assert_finite(loss, "loss")
        assert abs(loss.item() - 2.01) < 1e-3

    def test_distortion_beta_anneal(self, agg):
        loss0 = agg(_out(2.0, distortion=torch.tensor(10.0)), step=0)
        assert_finite(loss0, "loss0")
        assert abs(loss0.item() - 2.0) < 1e-3
        warmup = agg.warmup_steps
        loss_w = agg(_out(2.0, distortion=torch.tensor(10.0)), step=warmup)
        assert_finite(loss_w, "loss_w")
        assert abs(loss_w.item() - 2.1) < 1e-3

    def test_subtracts_routing_entropy(self, agg):
        loss = agg(_out(2.0, route_entropy=torch.tensor(1.0)), step=0)
        assert_finite(loss, "loss")
        assert abs(loss.item() - 1.99) < 1e-3
